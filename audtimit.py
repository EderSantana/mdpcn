...
class AUDTIMIT(dense_design_matrix.DenseDesignMatrix):
    """
    .. todo::
    ...
    def __init__(self, which_set, center=False, rescale=False, gcn=None,
        ...
        for i, fname in enumerate(fnames):
            ...
            if nloaded >= ntrain + nvalid + ntest:
                break
        '''
        path = os.path.join(serial.preprocess('${VIDTIMIT}'), 'data', 'audio_dataset.pkl')
        dataset = cPickle.load(file(path))

        # process this data
        train_idx = dataset['train_idx']==1
        test_idx = dataset['test_idx']==1
        Xs = {'train': dataset['data'][train_idx].reshape((-1,54*1000)),
              'test': dataset['data'][test_idx].reshape((-1,54*1000))}
        ...
        view_converter = dense_design_matrix.DefaultViewConverter((1000, 1, 54),
                                                                  axes)
        
        super(AUDTIMIT, self).__init__(X=X, y=y, view_converter=view_converter,
